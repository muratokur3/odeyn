...
- **Hayalet Kullanıcı ve Sahiplenme (Claiming):** Sisteme kayıtlı olmayan bir numaraya borç yazıldığında, sistem bu numarayı geçici kimlik (Placeholder) olarak kullanır. Kullanıcı gerçek hesabı oluşturduğunda veya giriş yaptığında 'Ghost User Protocol' (Claiming) tetiklenir; eski telefon referansları yeni UID ile değiştirilerek tüm tarihçe otomatik olarak kullanıcıya bağlanır.

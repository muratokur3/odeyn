...
import { ArrowLeft, FolderOpen, MoreVertical, Edit2, UserPlus, Volume2, VolumeX, Ban, Trash2, Receipt, History, CheckCircle, EyeOff } from 'lucide-react';
...
// Tab Mode Type
type TabMode = 'DEBTS' | 'LEDGER' | 'HISTORY';
...
export const PersonStream = () => {
    ...
    const [tabMode, setTabMode] = useState<TabMode>(() => {
        return (localStorage.getItem(`tabMode_${id}`) as TabMode) || 'DEBTS';
    });

    // Backwards compatibility: keep viewMode for existing summary cards
    const [viewMode, setViewMode] = useState<'FLOW' | 'SPECIAL' | 'TOTAL'>('FLOW');
